...
parser = argparse.ArgumentParser(
    description="Use the arguments to pass the token and camera path to the script, can either use just json or the rest of them"
)
parser.add_argument("-t", "--token", help="Token created by Prusa Connect")
parser.add_argument(
    "-n", "--name", help="Printer name to assist in debugging", default="printer"
)
parser.add_argument(
    "-f", "--fingerprint", help="Unique fingerprint >16 characters long"
)
parser.add_argument(
    "-i", "--ip", help="Local IP address of the printer to check print status"
)
parser.add_argument(
    "-k",
    "--apikey",
    help="PrusaLink API key, found on printer settings page of prusa connect",
)
parser.add_argument(
    "-d", "--directory", help="Absolute path to directory where to store images"
)
parser.add_argument(
    "-m",
    "--maximages",
    help="Maximum number of images for this camera to store in image folder",
    default=DEFAULT_MAX_IMAGES,
)
parser.add_argument(
    "-j", "--json", help="Absolute file path to configuration json file", default=None
)
parser.add_argument(
    "-r",
    "--rotate",
    help="How much to rotate the image by, needs to be a multiple of 90, optional",
    default=0,
)
parser.add_argument(
    "-s",
    "--sleep",
    help="How long to sleep between checks of the printer status",
    default=60,
)


def putImage(
    token: str, fingerprint: str, img_path: pathlib.Path
    ...
    try:
        ...
        if resp.status_code == 200:  # Successful upload of image
            ...
            logger.exception(
                f"Put Image: Response Code {resp.status_code}. Content: {resp.content.decode()}"
            )
        ...
        logger.warn(
            "Put Image: Connection Timeout. Meaning {URL} could not be accessed"
        )
        return None
...
def getPrinterStatus(ip: str, api_key: str) -> dict:
    ...
    try:
        resp = requests.get(
            url=f"http://{ip}/api/v1/status", headers={"x-api-key": api_key}
        )
        ...
        if resp.status_code == 200:
            ...
            logger.exception(
                f"Printer Status: Response Code {resp.status_code}. Content: {resp.content.decode()}"
            )
            raise ConnectionError(
                f"Printer Status: Response Code {resp.status_code}. Content: {resp.content.decode()}"
            )
...
def captureImage(
    camera_id: int | str, fingerprint: str, imgs_folder: pathlib.Path, rotation: int
) -> pathlib.Path:
    ...
    if cap.isOpened():
        ...
        if ret == True:
            file_name = (
                f"{fingerprint}_{datetime.datetime.now().strftime(TIMESTAMP_FMT)}.jpg"
            )
    ...
    if len(imgs) > max_images:
        sorted_imgs = sorted(
            ...
            key=lambda x: datetime.datetime.strptime(
                x.stem[len(fingerprint) + 1 :], TIMESTAMP_FMT
            ),
        )
...
if __name__ == "__main__":
    ...
    if args.json is not None:
        ...
        try:
            ...
            if int(config["rotate"] / 90) == config["rotate"] / 90:
                ...
                raise TypeError(
                    f"User input ({config['rotate']}) is not allowed, needs to be a multiple of 90"
                )
        ...
        if imgs_folder.exists():
            if imgs_folder.is_file():
                raise FileExistsError(
                    "Directory input already exists as a file, needs to be a folder"
                )
        ...
        try:
            ...
            if ret is False:
                raise ConnectionError(
                    "Argument supplied camera path is invalid, please select the camera manually by not passing in argument to -c or --camera or try a different absolute path. \n Sometimes cameras create multiple v4l devices so try other indicies (see readme)"
                )
    ...
    ##JSON args is not passed
        ...
        if int(args.rotate) == float(args.rotate):
            if int(int(args.rotate) / 90) == int(args.rotate) / 90:
                ...
                raise TypeError(
                    f"User input ({args.rotate}) is not allowed, needs to be a multiple of 90"
                )
    ...
    while True:
        ...
        while printer_status == "PRINTING":
            ...
            if (
                status is not None
            ):  # If the status check works properly change the state, otherwise do nothing
                printer_status = status["printer"]["state"]
